...
##'
##' @param orderObject An object of class \code{orderObject} that contain
##' s the processed data and the ordering.
##' @param plotFAST Logical, setting to FALSE will generate a higher quality 
##' plot.
##' @param blankWidth Indicates the amount of space to leave between the two 
##' plots
##' @param Title The title of the plot.
##' @param drawLine Logical, indicates whether to draw a line above the CG/GC 
##' sites.
##' @param drawKey Logical, indicates whether to draw a key representing a 
##' 147bp nucleosome at the bottom of the plot.
##'
...
plotSequence <- function(orderObject, plotFAST=TRUE,
    ...
    sites <- which(apply(abs(toPlot.fix.og), 2, 
    ...
    if(drawKey == TRUE) {
           blankROW <- matrix(rep(0, ncol(toPlot.fix)*12), nrow=12,
                              ncol=ncol(toPlot.fix))
    ...
    plot1 <- round(ncol(input.HCG.fix)/ncol(toPlot.fix), 2) # convert these 
    ##back to the site number so we can do refinement
    plot2 <- round((ncol(input.HCG.fix)+blankWidth)/ncol(toPlot.fix), 2)

    title("HCG", adj=plot1 / 2 - 0.025) # these shifts of 0.025 seem
    ##arbitrary but tend to center the title a bit better
    ...
    if (!(length(order1) %in% toLabel)) toLabel <- c(length(order1), 
                                                     toLabel)
    y.axis.starting.point <- ifelse(drawKey, nrow(blankROW) / nrow(toPlot.fix),
                                    0)
    axis(2, at=seq(y.axis.starting.point, 1, length.out=length(toLabel)),
         labels=toLabel)
